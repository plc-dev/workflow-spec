...
- [ ] 1.1 Confirm COW/incremental-snapshot capability for the SQL-execution service end to end (proof of concept); SQL-Service-image: ghcr.io/htw-aladin/sql-assessment-service:sha-23e9468 ; SQL-Service-Code: https://github.com/HTW-ALADIN/ALADIN-Services/tree/master/services/sql-assessment-service
...
- [ ] 1.2c (Deferred, not actively spiked) Dapr (Workflows + Actors) in placement-bookkeeping-only mode - documented as a fallback per design.md D6; only spike if 1.2 and the Restate evaluation (1.2a) together surface a blocker neither resolves - unchanged trigger condition, has not fired. Its main differentiator (a native Secrets API) is a nice-to-have against an already-decided, broker-agnostic D7, not something a spike needs to validate to make progress
...
- [ ] 1.4 Decide the execution engine (design.md D6, open question) - resolve alongside 1.6 (secrets-broker fit, e.g. Dapr's native Secrets API) and 1.9 (composability cost), since both depend on the engine chosen. Per design.md D6's updated evaluation, the Postgres-native path (resonate-pg-shaped fork) is the leading candidate pending 1.2e's load/scale check; nothing in 1.2a/1.2d is expected to overturn that, only to close out remaining desk-research items
...
- [ ] 1.10 Design the placement-resolver/routing mechanism (design.md D4/D6 Open Questions): bespoke resolver, service-mesh consistent-hash policy, or a native engine primitive, depending on the outcome of 1.4. If 1.4 selects the Postgres-native path, spike 1.2's `placement` table (content_hash -> replica_id, upserted in the same transaction as the durability core and session log) is a working draft of the bespoke-resolver option, not a from-scratch design - formalizing it (capacity/promotion fields for 4.x) should be materially faster than the other two options
