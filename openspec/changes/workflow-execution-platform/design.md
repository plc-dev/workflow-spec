...
- **Secrets broker product**: The secrets injection model is decided (D7), but the specific broker/store (e.g. Vault/OpenBao, Infisical, a cloud secret manager, or an encrypted-at-rest store decrypted worker-side) is intentionally left open (tasks.md 1.6). The spec is written broker-agnostic; a concrete comparison against D7's actual requirements (push-by-value, session-TTL-bound user secrets, writer/session namespace isolation enforced by the broker itself, not just application convention) narrowed this to Infisical or OpenBao as the two live contenders, pending a final pick.
