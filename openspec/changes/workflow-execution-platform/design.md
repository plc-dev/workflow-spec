...
**Companion software-design record.** `docs/adr/` holds the Architecture Decision Records that translate this design's conceptual decisions (D1-D17) into an actual repository structure, module boundaries, and internal contracts (repository/package layout, the execution plan-as-spine contract, binding resolution, the CLI-dispatch mechanism, the in-pod exec-agent, and the language/build/CI tooling). Where an ADR's decision has a direct product/onboarding consequence rather than staying a pure implementation detail (see D17a below), it is cross-referenced back into this document rather than left only in `docs/adr/`.

**Note on paths cited throughout D1-D17 below.** Every bare path this document cites as spike/prior-art evidence (`spikes/1.x-.../FINDINGS.md`, `registry/`, `placement-resolver/`, `dsl/schema/`) now lives under `archive/` (e.g. `archive/spikes/1.2-.../FINDINGS.md`) - moved there, unmodified, when the software-design ADR series was written; see `docs/adr/README.md`'s "Status of prior art." These citations are left as historical evidence of what was found, not pointers to where the real implementation lives going forward - the ADR series and tasks.md's module inventory are the current source of truth for that.
...
- Separate the DSL's authoring surface from a stable intermediate representation (execution plan), so authoring syntax can evolve or be plural without destabilizing the runtime contract, and so the underlying execution engine can be selected - or changed - independently of the DSL.
- Support real-world control-flow needs (conditional branching, dynamic-cardinality iteration, and bounded agent-directed composition) while keeping the execution plan as statically analyzable as possible for the scheduler.
- Resolve a request-scoped assessment-item identifier (supplied by an external digital learning environment, alongside interactively-produced learner data) against a trusted, external Item Pool into ordinary static-dataset bindings, reusing D1/D2's existing content-addressing/caching model rather than introducing a parallel one (see D15-D17).
...
- Modeling the Item Pool's own item-authoring UX, variance/difficulty-instance assignment logic, or item versioning semantics (D15) - these are entirely the Item Pool's concern; this design specifies only the platform-side resolution/mirroring boundary.
- Enforcing item-level confidentiality/visibility (e.g. draft or exam-sensitive item content) as a platform invariant (D16) - delegated entirely to the Item Pool, mirroring D13's existing delegation of workflow-spec visibility/tenancy to the external authoring tool.
- Selecting the execution engine itself (D6's leading paths are Temporal+resolver, Temporal+Ray hybrid, Restate, Dapr, or a DBOS-shaped self-written alternative; Argo scores weakly across D6's own evaluation and is not among the leading paths, though not formally excluded). Every requirement, execution plan construct, and spec in this change is written to be engine-agnostic so this decision can be resolved later without invalidating what's captured so far.
...
**Note**: this decision establishes *that* placement is fused from three sources and *that* affinity is optional, but deliberately does not specify the concrete mechanism that turns a placement decision into an actually-routed call to a specific service replica. **Update (task 1.10): this mechanism is now formalized.** The bespoke-resolver option (`archive/placement-resolver/`, now split into `core/`'s placement schema and `scheduler/`'s decision logic per `docs/adr/0002`/`0007`) implements D4a's cache-admission model concretely - `resolvePlacement`/`recordAccess`/`evaluatePromotion`/`evaluateDemotion`/`evictLRUIfOverCapacity` against a Postgres schema whose thresholds are tunable data (`placement_config`), not hardcoded constants - verified against a real Postgres instance (batch-never-promotes, promotion/demotion hysteresis, and pinned-only LRU eviction all hold). See `archive/placement-resolver/FINDINGS.md` for the full design.

**Further update (`docs/adr/0008`): the "actually-routed call" half is now formalized too, distinct from the "which replica" half above.** Task 1.10's resolver answers *which* replica is warm for a given content hash; it does not by itself specify *how* the worker turns that answer into a real invocation. `docs/adr/0008-in-pod-exec-agent.md` closes that remaining gap for the CLI-dispatch mechanism decided in D17a: the worker addresses a specific pinned replica's in-pod exec-agent directly (by Pod IP or headless-service DNS), falling back to any replica on a miss - the same rehydrate-anywhere fallback this section already requires, now with a concrete transport.
...
**What this decision does NOT yet settle** (tracked as their own tasks, not blocked on re-opening this decision): 1.9 (service-nesting mechanism cost, coupled to this choice). 1.10 (placement-resolver), 1.5 (generic execution plan interpreter), and 1.6 (secrets-broker product) are now all done - see `placement-resolver/FINDINGS.md`, `spikes/1.5-ir-interpreter/FINDINGS.md`, and `spikes/1.6-secrets-broker-eval/FINDINGS.md` respectively.
...
(e) Conductor (Orkes) as a single, Netflix/enterprise-proven system with a
    ...
    lowest-effort execution plan-to-engine compilation target of any candidate. Does not
...
DONE - LIGHT-TOUCH CONDUCTOR EVALUATION (its native MCP gateway's fit for
  D9c and D8's execution plan-to-engine compilation step): substantiates, rather than
...
**Alternatives considered**: Environment-variable injection (rejected - incompatible with pooling, per rule 2). Capability-token / pull-from-broker-by-the-container model (deferred - stronger, but solves an active-exfiltration threat that does not exist for trusted services; revisit if untrusted service images are ever allowed). Passing secrets as workflow arguments (rejected - persists them in durable history). The secrets-broker product question (previously left open here) is now decided - see task 1.6 below.

**Secrets-broker product: DECIDED (task 1.6): OpenBao.** Narrowed to Infisical vs. OpenBao by a prior comparison (see the former Open Questions entry, now resolved); the deciding factor is rule 5 above - session-TTL-bound, broker-enforced secret revocation is a native, free-core capability in OpenBao (lease/TTL/expiration-manager mechanics inherited from its Vault lineage) but sits behind Infisical's paid `ee/` tier (Dynamic Secrets) even when self-hosting, which would otherwise force either a licensing cost or falling back to an application-level expiry sweep - precisely the "not just application convention" case this rule's wording was written to exclude. OpenBao's path-based ACL policies and free multi-tenant Namespaces also give the writer/session isolation boundary a more structural home than Infisical's core tier. Accepted cost: OpenBao is the operationally heavier of the two to self-host (its own storage backend, unseal/init lifecycle, no built-in dashboard) - accepted as consistent with this platform's existing posture of self-operating its own Postgres-native durability layer (D6/1.2e), rather than a new category of operational cost. See `spikes/1.6-secrets-broker-eval/FINDINGS.md` for the full comparison against D7's specific requirements (not a general feature survey).

### D8: DSL splits into an authoring surface and a stable execution plan; the execution plan has static shape but dynamic cardinality/path

The DSL is deliberately split into two layers:

```
   AUTHORING SURFACE (what a writer types)  --synthesize once-->  execution plan (what runs)
...
**Rationale**: Separating authoring syntax from a stable execution plan avoids over-constraining the DSL's surface syntax by any one runtime engine's determinism requirements, and keeps "static, mostly-static, or dynamic" a property of specific execution plan constructs (branch, map) rather than an all-or-nothing property of the whole graph.
...
Resolves the "authoring surface syntax" open question left by D8, worked out by writing a full example workflow-spec against every execution plan construct decided so far - which surfaced several refinements the abstract execution plan description hadn't captured.
...
**Rationale**: Every refinement here was found by attempting to write a complete, concrete example against the already-decided abstract execution plan, rather than staying at the abstract level - confirming that concretizing syntax is a genuine design activity, not just a mechanical transcription of prior decisions.
...
**The workflow-spec version field is `workflowSpecVersion`, not merely an example name.** D11 introduced it as "e.g. `workflowSpecVersion: N`"; every worked example since D8c has used it by convention. This decision locks it as the actual field name, at the top level of a `WorkflowSpec` document.
...
**Rationale**: All four follow the same shape as this session's other syntax-audit findings - checking whether a candidate construct needs new grammar, and finding either that it structurally doesn't (nesting depth, error-handling scope), that it's explicitly excluded as a scope boundary (retry/timeout, error-handling), or that it merely needed a "e.g." upgraded to a decision (`workflowSpecVersion`).

**Alternatives considered**: Author-configurable per-step retry/timeout (rejected for now - no concrete need identified; platform-uniform defaults are simpler and can be relaxed later without breaking existing workflow-specs, since adding an optional field is backward-compatible under D11's migration model). A DSL-level catch/compensation construct (rejected as out of scope - a real feature, but a substantially larger one than this design set out to cover; native engine retry is deemed sufficient for this design's scope). Leaving `workflowSpecVersion` as a non-binding example rather than locking it (rejected - every artifact captured since D8c already uses it as if decided; leaving it formally open served no purpose). A fixed nesting-depth limit (rejected - no evidence yet that deep nesting causes a real problem; premature to constrain without one).
...
**Task 1.9 DECIDED, now that D6/D6a resolves the mechanism-cost question above: no bespoke SDK is required; HTTP/CLI/MCP are three thin transport projections of one dispatch primitive.** The Temporal-vs-Restate/Dapr contrast this section drew is moot for a clean-room engine we built ourselves (D6a) - there is no third-party client protocol to adopt or avoid, because there is no third party. Concretely: a nesting service's orchestrator-aware call to another registered service is an ordinary POST to the platform's own dispatch endpoint (or an equivalent CLI invocation or MCP tool call) - never a call to the target's raw endpoint - and that ONE dispatch primitive is what performs the tracked-child-execution insert (the same `executions`/`checkpoints` pattern spikes 1.2/1.5/1.8 already validated). HTTP, CLI, and MCP are three thin projections of that single primitive, exactly mirroring D9c's already-decided framing for the *target* side of a nested call ("REST, CLI, and MCP become three projections of the same registry entry + OpenAPI contract") - this resolves the *calling* side the same way. A service satisfies "orchestrator-aware by default" with nothing more than an ordinary HTTP client, CLI, or MCP call to a well-known internal address - tooling every language already has, not a workflow-engine-specific package to learn. `sdk` remains a listed, valid value in the registry's `nesting_declaration.via` enum (already implemented and validated, `registry/src/validate.js`'s `NESTING_TRANSPORTS`) - not because it is required, but because nothing prevents an optional, generated, thin client wrapper over the same dispatch endpoint from existing for services that prefer a typed convenience layer. Declaring `via: sdk` would mean "this service happens to use our optional convenience client," never "this service requires a bespoke SDK to comply" - the distinction that mattered under Temporal doesn't survive the move to a clean-room engine we fully control.
...
**D9c: Agent-directed nesting is the `targets: open, via: mcp` case of D9b - not a separate execution plan construct.** An "agent-directed step" is not a distinct kind of step. It is an ordinary step invoking an ordinary registered service (an "agent-runner") whose own capability declaration is `nesting_declaration: { via: mcp, targets: open }`. This collapses what earlier looked like bespoke agent machinery into the generic model:

```
   ALLOWLIST AND GOVERNOR ARE ORDINARY REQUIRED PARAMETERS, not new execution plan:
...
**Rationale**: Reuses a principle already present elsewhere in this design (D4 pushes mechanism into the scheduler, D5 pushes capability facts into the registry) - here, pure computation is pushed into a bounded expression rather than into either the execution plan's control-flow grammar (which stays free of a general expression language) or a proliferation of trivial services.

**Alternatives considered**: A general expression/scripting language embedded in the execution plan (rejected - reintroduces the decomposability problem D9's code prohibition was meant to avoid, just with a friendlier syntax). A mandatory registered "utility service" for every simple comparison/transform (rejected as the default - unnecessary operational overhead and a step-execution round-trip for what is often a single comparison; remains available for the non-pure cases it's actually suited to). CEL (rejected in favor of JSON-Logic per the comparison above, primarily on UI-decomposability grounds).

### D11: workflow-spec schema versioning is a whole-document tag, migrated lazily and forward-only

The funded UI project has its own, independently-timed release cadence, separate from the platform backend - meaning the UI could plausibly lag behind whatever workflow-spec version the backend is producing at any given moment. This is a structural consequence of two funded workstreams moving independently, not a hypothetical edge case, and it is why versioning needs an explicit answer now rather than being deferred until the UI project starts.

```
VERSION TAG: a single, whole-document version field, `workflowSpecVersion: N`
...
**Rationale**: Answers the "workflow-spec schema versioning/migration" open question left by D8, specifically motivated by the UI project's independent timeline rather than treated as a generic best practice to defer indefinitely.

**Alternatives considered**: Per-construct versioning (rejected for now - adds complexity with no clear benefit while the workflow-spec is authored/synthesized as a whole document, not assembled from independently-versioned fragments). Kubernetes-style multi-version serving (rejected as premature - the operational cost isn't justified until there's a concrete need for the runtime to serve more than one workflow-spec version at a time).

**Note (terminology amendment): `workflowSpecVersion` is author-facing, not an execution-plan version.** This field versions the *workflow-spec* - the document a workflow-writer authors and the UI opens/edits, which is what this decision's own motivation (the UI's independently-timed release cadence) is actually about. It was originally named `irVersion`; renamed here to make that author-facing scope explicit (see `docs/glossary.md`). Today the execution plan the engine runs is byte-identical to the workflow-spec (no `dsl-compiler/` exists yet - see ADR-0003's terminology note), so `workflowSpecVersion` is, for now, the only version tag in play. Once a future compiler splits a distinct execution plan out of the workflow-spec (tasks 5.2/5.10), execution-plan versioning becomes its own, separate concern - not necessarily bumped in lockstep with `workflowSpecVersion` - and should be designed as its own decision at that point rather than assumed to inherit this one.
...
```
   ...
   contents               service-image metadata      workflow-specs (execution plan + doc)
...
WORKFLOW-SPEC-VERSION MISMATCH ON FORK: a source workflow-spec and the forking
author may be on different workflow-spec versions (D11). This decision does not
...
### D14: Identity is asserted at the platform boundary, not issued by the platform; writer identity is an opaque `writerId`, end-user identity is never a platform concept at all

Two prior decisions each depend on an identity the platform can trust - D7 scopes secrets to "the workflow-spec / writer identity" (main case) or "the session" (secondary case); D13 delegates "who may see, fork, or publish into a namespace" to the external authoring tool, which is meaningless without *some* notion of who is asking. Neither decision specified how that identity is established or how it reaches the platform's APIs - an implicit assumption surfaced by tracing a concrete secrets scenario end to end (a workflow-writer registering a writer-scoped API key; an end-user connecting their own account at session start), not a hypothetical gap.

**Tracing the two cases separately reveals they are not the same problem.** D7's own scope table already says so, easy to miss on a first read: a writer-scoped secret's isolation boundary is "the workflow-spec / writer identity" and its lifetime is *every run, indefinitely*; a user-scoped secret's isolation boundary is *the session* and its lifetime is *only that session*. The second row means **user-scoped secrets never required a durable, cross-session end-user identity in the first place** - only a session id, which D2/D3 already mint as a first-class concept. If an end-user starts a new session tomorrow, this design's own scoping already implies they reconnect their account again, unless some system entirely outside this platform (the front-end/product layer) chooses to remember them and re-provision a new session's secret using a longer-lived credential *it* holds - a UX choice for that other system, not a gap this decision needs to close. Writer identity has no equivalent escape: it has to be durable, because a writer-scoped secret must resolve identically across every run of that workflow-spec, forever.

**Decision: the platform is a relying party for identity, never an issuer of it.** This is the direct extension of a layering principle D13 already established ("the platform enforces invariants; the external authoring tool owns policy and UX") applied to identity itself, rather than a new principle invented for this decision:

```
WRITER IDENTITY: established and authenticated entirely OUTSIDE the platform,
  by whatever system the external authoring tool (D8/D13) already needs for
  its own login/tenancy UX - the platform never runs its own signup/login.
  The authoring tool presents an already-authenticated identity assertion
  (e.g. a bearer token) on every call that needs one; the platform's only
  job is to VERIFY that assertion at its API boundary and extract a stable
  subject identifier from it. That subject identifier IS the `writerId` -
  an OPAQUE STRING, nothing more. The platform does NOT maintain its own
  `writers` table, profile data, or account model of any kind: `writerId`
  is used purely as a scoping key (the namespace segment of a workflow-spec
  URN per D13; the `writer/<writerId>/<name>` path segment in the secrets
  broker per D7) - never a joined-against, platform-owned entity.

END-USER IDENTITY: not a platform concept at all. Only SESSION identity
  (D2/D3, already decided, already minted by the platform at session
  start) is needed anywhere D7 or D13 currently require "the end-user."
  A user-scoped secret is created against `session/<sessionId>/<name>`,
  by whatever front-end flow the end-user completes (e.g. an OAuth
  connect flow) - the platform is handed a session id it already owns,
  not a separate user id it would have to newly define, verify, or store.

CONSEQUENCE FOR BOTH: the platform holds no password, no login flow, no
  durable "user" or "writer" profile table - identity flows IN, as an
  already-established fact the platform verifies and scopes by, never
  something the platform itself establishes.
```

**What this decision explicitly does NOT settle** (real open questions, distinct from the layering principle above, which IS settled):

1. **The concrete identity-assertion mechanism at the platform's API boundary** - JWT verified against an external issuer's JWKS endpoint is the most standard fit for "an external tool asserts an identity, we verify it," but a pre-shared per-writer API key or mTLS-based client identity remain possible alternatives. Not decided here.
2. **Whether an identity provider already exists in the broader product context** (the funded UI/authoring tool project, or the wider ecosystem the pinned SQL-execution service in task 1.1 is drawn from) that this should integrate with, rather than a fresh one being stood up for this purpose. Not established one way or the other by anything researched so far - a real question for whoever owns that external project, not something this design can answer from inside the platform's own scope.
3. **Where JWKS/issuer configuration and token-verification middleware actually live** in the eventual service topology (a shared piece of infrastructure every platform API uses, presumably, but not yet placed as its own task).

**Rationale**: Mirrors D7/D13's already-established pattern (D4/D4a: the safe/default path is decided even when the mechanism cost is engine-dependent; D13: the platform enforces invariants, the external tool owns policy) - applying it here means the *shape* of the identity boundary (relying party, opaque `writerId`, no platform-owned user concept) is decidable now, while the specific verification mechanism and any external-IdP integration remain open, tracked questions rather than silently-assumed answers.

**Alternatives considered**: The platform running its own signup/login and maintaining a `writers`/`users` table (rejected - duplicates identity/tenancy machinery the external authoring tool already needs to own per D13, and there is no requirement anywhere in this design that the platform itself know anything about a writer beyond a stable scoping key). Giving end-users a persistent, cross-session platform-level account (rejected - D7's own scope table only ever required session-level isolation for user-scoped secrets; inventing a durable end-user identity would be solving a problem this design was never asked to solve, and would belong to the front-end/product layer if ever needed). Treating `writerId` as a foreign key into a platform-owned entity with profile/metadata (rejected for now - nothing in D7 or D13 needs more than an opaque scoping string; revisit only if a real need for platform-owned writer metadata emerges).

### D15: Assessment items are static-dataset content resolved through an external Item Pool; item type (authored) vs. item instance (request-resolved)

An external digital learning environment invokes workflows on behalf of students, passing an assessment-item identifier alongside interactively-produced learner data (e.g. a submitted SQL query). The assessment item's own content (a database dump, a reference-solution, a rubric) is not a new kind of data - it is exactly D1/D2's existing `static` scope (shared, content-addressable, worth caching/pooling), simplified away too early in this design's earlier framing. What is genuinely new is *when* the concrete reference is chosen:

```
ITEM TYPE   - authored, like a contract/schema. A workflow-writer (typically
              a teacher, who is also the likely item author) authors a
              workflow already knowing an item type's resource shape (e.g.
              "this item type has a dump and a referenceSolution"), the
              same way they'd know a dataset's shape before binding it.

ITEM INSTANCE - one of many variance/difficulty instances of the same type,
              chosen per learner by the learning environment/Item Pool, NOT
              by the workflow-writer at authoring time. Only the INSTANCE
              identifier is request-scoped; the workflow-spec itself never
              names a specific instance.
```

One workflow-spec is only ever bound to a single item instance per run; it may legitimately be reused, across separate runs, with different instances of the same item type, or even with a different item type entirely - but that reuse decision belongs entirely to the learning environment invoking the workflow, and is invisible to (not a concern of) this platform, the same way D8a already treats a workflow-spec's derived signature as an opaque contract from the caller's point of view.

**Resolution is eager, whole-item, and happens synchronously in the student's request path.** On first reference to a given item instance anywhere on the platform, the ENTIRE manifest the Item Pool returns for that instance is fetched and flattened in one pass (not resolved lazily, path-by-path) - accepted as the simpler option even though it may occasionally flatten resources a given workflow-spec doesn't end up using. The added latency and availability coupling to an external system this introduces is accepted as an ordinary step-shaped cost (native retry/backoff per D6/R7 already covers transient Item Pool unavailability), not something requiring pre-fetch/pre-warming machinery.

**Resolution is a derived, GC-able cache over an external durable source of truth - the same move as D2/D3/D6, applied a further time:**

```
D3:  user input history (durable, platform-internal)     -> session snapshot cache
D6:  Postgres durability log (durable, platform-internal) -> everything else consolidates on it
D15: Item Pool's own item data (durable, EXTERNAL)        -> URN-keyed dataset-catalog entries
```

Concretely: `(itemInstanceId)` resolves, on first touch, to a flat map of `path -> { kind: dataset, ref: <urn> } | { kind: value, value: <...> }`, memoized exactly like D2's `(input-hash, operation) -> output-hash` cache - safe to TTL/GC and rebuild by re-querying the Item Pool, because the Item Pool (not this cache) is the actual source of truth. Instance-to-resource-map stability is required only for the life of one learner attempt/session (confirmed: an instance is stable within one attempt) - it is not a global invariant across attempts, so this cache does not need to outlive a session's own retention policy (D3a) to be correct, only to be an optimization within one.

**Resource weight varies per named resource within the same item type, and is only known once the manifest is actually fetched** - a reference-solution might be a plain string (light, no dataset-catalog entry needed, passed through like a `literal`/step-output value) while a database dump is heavy (D1's "tens of GB" cell, needs a real URN, D1/D2/D4/D5's full materialization/placement/pooling machinery). See D16 for how a leaf's weight is classified during flattening and what depending on the Item Pool's self-description of that classification implies.

**Rationale**: Reframing the assessment-item-identifier problem as "a `static` binding whose *ref* is late-bound at request time, rather than author time" - instead of as a wholly new resource-discovery subsystem - means every downstream materialization/caching/pooling/trust decision already made in D1-D5 applies unchanged; the only genuinely new mechanism is the resolution/flattening step itself and the new binding kind that triggers it (D16).

**Alternatives considered**: Lazy, per-referenced-path resolution instead of eager whole-item flattening (rejected for now as unneeded complexity - simpler to flatten the whole manifest once per instance; revisit only if a real item type's manifest turns out large enough, or reused across enough different workflow-specs with disjoint path needs, that eager flattening's waste becomes a measured problem). Pushing item resolution entirely outside the platform, with the learning environment resolving to URNs before ever calling this platform (rejected - the confirmed need to "flatten into URN-keyed resources" means *someone* still mirrors Item Pool bytes into this platform's own dataset catalog; keeping that inside the platform, next to the D1/D2 machinery it feeds, avoids duplicating that logic in an unspecified adapter outside this design's scope). Treating item resolution as an ordinary registered service step whose output is consumed like any other step-output binding (rejected - would lose D1/D2/D4/D5's shared/pooled/COW treatment for the heavy leaves, since a plain step-output binding carries no scope-classification the placement layer can act on; D16's dedicated binding kind exists specifically to preserve that classification).

### D16: A dedicated `itemResource` binding kind, arbitrarily-nested path addressing, and item confidentiality is delegated entirely upstream

**A new execution plan binding kind, `itemResource`,** resolves against D15's per-instance flattened cache: `{ from: itemResource, itemId: <a binding, typically request-scoped>, path: <locator into the flattened manifest> }`. At run time it resolves to either an ordinary `static`-equivalent dataset reference (heavy leaf) or a plain passed-through value (light leaf), per D15's flattening classification - the workflow-spec author does not declare which in advance; the platform decides based on what the Item Pool's manifest actually contains for that path.

**Path addressing is arbitrary and deeply nested, using a locator syntax (e.g. JSON Pointer, RFC 6901) rather than a new expression language.** This deliberately differs from D8a's flat-only rule for `request` parameters ("never a path into an arbitrary nested request body") - that rule governs extracting a value at *execution time* from an already-bound in-memory value (reusing JSON-Logic's `var`), whereas an `itemResource` path *addresses into an externally-sourced document to decide what gets bound in the first place*, the same job D8a's own dataset URN already does (a locator/reference syntax, not a runtime expression). Keeping the item type's resource shape internally consistent with what a workflow-spec's `itemResource` paths expect is explicitly **not** a platform-enforced invariant - it is delegated upstream to the external authoring tool and/or the Item Pool's own item-type authoring flow, the same delegation pattern D13 already uses for workflow-spec visibility/tenancy. A shape mismatch (a path that doesn't exist in a given instance's manifest) is therefore a run-time resolution failure, not something the DSL can reject at compile time - a deliberate, not accidental, gap.

**Item confidentiality is the Item Pool's concern, not the platform's - a further application of D13's "no tenancy" posture, not an exception to it.** D13 already decided static datasets are globally shared by design, with no ACL/tenancy model, specifically to keep that machinery out of the platform. Assessment items could have broken that assumption (exam content, unpublished items, per-institution restrictions) if the platform had to enforce it - but it does not have to: the Item Pool only ever hands this platform a manifest it was already authorized to resolve, so once mirrored into this platform's own dataset catalog, an item-derived resource is an ordinary globally-shared-within-the-platform dataset, exactly like `northwind`. The platform remains a trusted-infrastructure boundary, not a multi-tenant dataset host, for item content the same way it already is for everything else D13 covers.

**The Item Pool's self-description of a leaf's weight (dataset-shaped vs. plain value) is trusted outright, not tiered.** Unlike D5a's per-service-image trust-tier model (which exists because arbitrary third-party service images are a real trust boundary), the Item Pool is confirmed to be trusted upstream infrastructure, not an unknown or adversarial actor - so no `unverified -> conformance-passed -> production-proven` progression is introduced for it. This is named explicitly, not left implicit, because the underlying stake (D6's R3: large state must move by handle, never by value) is a correctness property, not merely a placement optimization - misclassifying a heavy leaf as a light value could mean literally inlining a multi-GB blob into a request body. The accepted mitigation is trusting the Item Pool's own self-description, not building a parallel trust-tier mechanism for it.

**Rationale**: Reuses three already-established patterns (D8a's locator-not-expression-language precedent for reference syntax, D13's delegate-policy-to-the-authoring-tool posture, and D13's globally-shared-by-design stance for static data) rather than inventing new mechanisms for a case that turns out structurally similar to problems already solved elsewhere in this design.

**Alternatives considered**: Reusing JSON-Logic's `var` operator for `itemResource` paths, matching D8a's `request`/`item` field-extraction mechanism exactly (rejected - that mechanism extracts from an already-bound in-memory value at execution time; `itemResource` addresses an externally-sourced document to decide what gets bound at all, a different job better matched by a locator syntax, the same distinction that already justifies the dataset URN's own existence). Enforcing item-type/workflow-spec shape consistency as a platform (DSL-compile-time) invariant (rejected per explicit direction - this is upstream tooling's job, mirroring D13's delegation). A per-item ACL/visibility model inside the platform's own dataset catalog (rejected per explicit direction - confidentiality is the Item Pool's job; the platform stays a trusted, non-tenanted infrastructure boundary for this the same as for every other static dataset).

### D17: CLI heavy-data transport is a mandated onboarding contract, not a discovered D5 capability

CLI-invoked functions cannot accept heavy data as an ordinary argument the way a REST body technically (if impractically) can - OS-level argv limits make a filesystem-path or a stdin stream the only real options once data crosses any real size threshold. Confirmed: CLI-invoked services here can genuinely be **warm** - a long-lived container, `exec`'d into repeatedly as separate process invocations that share the container's persistent local filesystem across calls (the CLI-world equivalent of the REST SQL service's in-memory `databaseId`-keyed reuse - state persists via local disk instead of process memory).

**Decision: a single, universal, platform-mandated calling shape for CLI heavy-data transport**, not a per-service discovered capability:

```
EVERY onboarded CLI-invoked function that accepts a heavy (dataset-scoped)
binding SHALL be invoked as:
    <cli-binary> ... --data-file <local-path> --state-id <key> ...

- <local-path> is resolved and made present by the platform BEFORE the
  exec call (mechanism - shared always-mounted volume, a per-hash CSI
  volume attached at D4a's promote step, or a worker-written local
  scratch file - is entirely the platform's business, invisible to the
  service author, the same separation D4 already draws between intent
  and mechanism elsewhere)
- <state-id> is the same content-hash-derived key D2 already mints;
  a function MAY persist local state keyed by it and reuse that state
  on a later call with the same key, without re-reading --data-file
```

This is a deliberate, named departure from D5's stated posture elsewhere ("capability is discovered, never assumed, because services are otherwise not dictated a common interface") - justified because it mirrors a precedent this design already relies on for onboarding, not a novel exception: the registry already mandates exactly one contract shape for OpenAPI (D12, "the sole stored contract"), never discovering whether a service happens to expose one. Mandating the heavy-data CLI calling convention the same way (a fixed onboarding requirement, not a probed fact) is consistent with that precedent, applied to one additional axis.

Because the convention is mandated rather than discovered, D5's per-function capability metadata is unaffected in scope - it still only needs to capture mutation/materialization-cost/COW/change-detection, exactly as already decided; "transport shape" never becomes a new discovered-capability axis at all.

**Volume/state lifecycle ties directly into D4a's existing promote/demote model, not a new lifecycle concept:** attaching whatever makes `--data-file`'s path resolve correctly happens at promotion (a replica becoming pinned to one content hash); detaching/deleting local state happens at demotion (the same hook that would otherwise just release memory for a REST-warm service now also cleans up a local state directory).

**Rationale**: A universal mandated shape, chosen over a small declared enum (e.g. `file` vs. `stdin`, service-selectable), for simplicity - accepted even though pure one-shot/stateless CLI functions that will never reuse a `--state-id` are handed a convention slightly heavier than they strictly need; revisit only if a real function's onboarding cost from the unused `--state-id` plumbing becomes a measured problem, not a hypothetical one.

**Alternatives considered**: A small closed enum of platform-blessed transport shapes a function declares among (`file` vs. `stdin`) (rejected in favor of the universal shape, for simplicity, per explicit direction - noted as a real, revisitable alternative if the one-size-cost above ever bites). Discovering transport shape per-service the way D5 discovers mutation/COW/change-detection (rejected - transport shape is an onboarding-contract question the platform can and should dictate outright, not a fact about pre-existing service behavior to probe for, unlike D5's other axes which describe behavior of images not built with this platform's contract in mind). Stdin-streaming as the universal default instead of a file path (rejected as the *universal* choice specifically because warm reuse needs state to persist somewhere between separate exec invocations, and disk is the natural place - stdin remains available as future work for functions that declare themselves non-reusable, under the closed-enum alternative above, if that alternative is ever revisited).

### D17a: CLI transport is now the universal step-dispatch mechanism, not a per-function-discovered choice - software-design elaboration, `docs/adr/0005`/`0008`

**Supersedes D17's scoping, not its mechanism.** D17 above mandated the `--data-file`/`--state-id` shape only for "every onboarded CLI-invoked function that accepts a heavy binding" - implicitly leaving open whether a given function is CLI-invoked at all, versus REST-invoked, as a per-function fact. The software-design ADR series (`docs/adr/0005-step-dispatch-is-cli-nesting-stays-flexible.md`, formalized further in `docs/adr/0008-in-pod-exec-agent.md`) resolves that implicit openness: **the engine invokes every step's target function via CLI, unconditionally** - never REST, regardless of whether that step's bindings are heavy or light. D17's mandated shape generalizes from "the heavy-data convention for CLI-invoked functions" to "the uniform binding-injection mechanism for every step": light bindings pass as ordinary CLI flags; heavy/dataset-scoped bindings use `--data-file`/`--state-id` exactly as D17 already specified. This is the same kind of scope-widening D17 itself already accepted in its own Rationale ("a universal mandated shape... accepted even though pure one-shot/stateless CLI functions... are handed a convention slightly heavier than they strictly need") - D17a widens *which functions* that universal shape applies to, not the shape itself.

This does **not** touch D9b/D9c's nesting model at all - a service's own outbound calls to *other* registered services remain governed by `nesting_declaration.via` (`sdk|http|cli|mcp`), unchanged. D17a is exclusively about the axis D9b/D9c never covered: how the engine itself invokes the step a workflow-spec actually declared.

**A concrete compatibility finding, since resolved.** The one real, already-identified external service this design has been validated against (`ghcr.io/htw-aladin/sql-assessment-service:sha-23e9468`, task 1.1) was confirmed, by direct probing, to be invoked **exclusively over its HTTP API** - `archive/spikes/1.1-cow-snapshot-poc/FINDINGS.md` documents every probe as an HTTP call (`curl ... -X POST`), and no CLI invocation path for this service was found or tested. This sat in tension with this proposal's own opening premise that every onboarded service is "REST API + CLI" - at minimum, this specific already-identified reference service had not been shown to have a CLI surface at all.

**RESOLVED: every onboarded service must ship a native CLI, no exceptions - no platform-generated wrapper substitutes for one.** The universal CLI-dispatch mandate (D17a above) is not walked back, and onboarding does not accept a REST-only image by wrapping it - a service that does not natively expose the mandated CLI contract is not onboardable as-is. **Direct, concrete consequence**: `ghcr.io/htw-aladin/sql-assessment-service:sha-23e9468`, the reference service task 1.1's spike was validated against, cannot be onboarded to this platform in its current, REST-only shipped form - it requires a real, upstream code change (adding a native CLI entrypoint satisfying the `--data-file`/`--state-id`-shaped contract) before this design's step-dispatch model can invoke it. This is recorded here as a real, actionable follow-up (see tasks.md 1.1's note and 2.3's backfill task), not a hypothetical - the same posture already applied elsewhere in this design to a negative finding (task 1.1's own COW result was itself "a legitimate negative result, not a failed spike").
...
- **[Item Pool is a new synchronous external dependency in the student's request path]** D15 accepts Item Pool resolution as an ordinary, latency-adding, availability-coupled call in the critical path, rather than requiring pre-fetch/pre-warming. -> Mitigation: resolution is memoized per item instance (D15), so only the first touch of a given instance anywhere on the platform pays the round trip; native retry/backoff (D6/R7) already covers transient Item Pool unavailability without new machinery.
- **[Trusting the Item Pool's leaf-weight self-description is a correctness dependency, not just a convenience]** D16 accepts the Item Pool's own classification of a resource as dataset-shaped vs. plain-value outright, with no tiered trust model - a misclassification could mean inlining heavy content where D6's R3 (by-handle, never by-value, for large state) requires a handle. -> Mitigation: accepted because the Item Pool is confirmed trusted upstream infrastructure, not an arbitrary third-party image; revisit only if item content is ever sourced from a less-trusted Item Pool implementation.
- **[Universal CLI heavy-data transport contract has an adoption cost for stateless functions]** D17's single mandated shape (`--data-file`/`--state-id`) hands even a pure one-shot CLI function a `--state-id` it will never reuse. -> Mitigation: accepted for simplicity; the closed-enum alternative (a function declares `file` vs. `stdin`) remains available to revisit if this cost is ever measured as a real onboarding friction, not just a theoretical one.
...
- **Identity-assertion mechanism (D14)**: D14 decides the *shape* of the identity boundary (platform as relying party; opaque `writerId`; no platform-owned end-user identity) but not the concrete verification mechanism - JWT+JWKS against an external issuer, a pre-shared per-writer API key, or mTLS remain open.
- **Whether an existing identity provider should be integrated with (D14)**: whether the funded UI/authoring-tool project (or the wider ecosystem the task 1.1 SQL-execution service is drawn from) already has an identity provider this platform should verify tokens against, rather than a new one being stood up - not established either way; a question for that project's owners, not decidable from inside this design.
...
- **workflow-spec-version-mismatch-on-fork (D13)**: surfacing a mismatch between a source workflow-spec's workflow-spec version and a forking author's own is delegated to the external authoring tool.
- **Item Pool integration surface/protocol (D15)**: the model (eager whole-item flattening, memoized per instance, mirrored into the existing dataset catalog) is decided; the concrete Item Pool API/protocol this platform integrates against, and which specific product/service backs "the Item Pool," are not specified here - tracked as an implementation-level detail once a concrete Item Pool is identified.
- **`itemResource` locator syntax's exact grammar (D16)**: JSON Pointer (RFC 6901) is the leading candidate for path addressing into a flattened item manifest, consistent with D8a's locator-not-expression-language precedent, but the exact grammar is not formally locked down here.
- **CLI heavy-data transport contract's exact flag names/schema (D17)**: the shape (`--data-file <path> --state-id <key>`) is decided as illustrative of the model; the literal flag names/argument schema are an implementation detail, not fixed by this design.

**Resolved this round** (previously listed here as open; concrete decisions now captured in the Decisions section above): execution engine selection - a Postgres-native, clean-room implementation of THE PATTERN, empirically confirmed via spikes 1.2/1.2-hatchet/1.2a/1.2d/1.2e, with the concrete fork-target question (resonate-pg vs. hatchet-dev's tutorial vs. clean-room) also resolved (D6/D6a); the placement-resolver/routing mechanism - a formalized bespoke resolver (`placement-resolver/`), since D6 resolved in favor of the Postgres-native path (D4/D6, task 1.10); nesting mechanism cost - no bespoke SDK required, since the engine D6 selected is one this team built and controls rather than a third party's client protocol (D9b's resolution note, task 1.9); the placement-bookkeeping-vs-actor-hosting trade-off this section used to flag - moot now that D6 didn't select an actor framework at all.

**Resolved this round** (previously listed here as open; concrete decisions now captured in the Decisions section above): JSON-Logic vs. CEL (D10); agent-directed/MCP nesting, unified as a specific case of the nesting model rather than a separate design (D9c); authoring surface syntax - YAML/JSON restricted profile, `sessionState` declarations, `{from: item}`, `dependsOn`, flat request signatures, and the URN scheme for static dataset references (D8a); workflow-spec schema versioning/migration (D11); snapshot auto-promotion thresholds (D4a); capability declaration trust/validation (D5a); session snapshot retention for undo/time-travel (D3a). The exact numeric defaults in D4a and the specific conformance-test implementation in D5a remain tunable/to-be-implemented, but the model itself is no longer open.

**Resolved this round** (previously listed here as open; concrete decisions now captured in the Decisions section above): the secrets-broker product (D7, task 1.6) - **OpenBao**, chosen over Infisical because D7 rule 5's session-TTL-bound, broker-enforced secret revocation is native and free in OpenBao's core but Enterprise-gated in Infisical; see `spikes/1.6-secrets-broker-eval/FINDINGS.md`.
